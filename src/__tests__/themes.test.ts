...
function parseCssRules(
  ...
  if (ruleMatches) {
    for (const rule of ruleMatches) {
      const [, selectorPart, propertiesPart] =
        rule.match(/([^{]+)\s*\{([^}]+)\}/) || [];
...
describe('Theme CSS Validation', () => {
  ...
  describe('Light Theme Validation', () => {
    it('should have correct base styles following Phase 6 specification', () => {
      ...
      // Check that font-family includes monospace fonts and ends with generic monospace
      expect(baseRule['font-family']).toContain('monospace');
      expect(baseRule['font-family']).toContain('Consolas');
      expect(baseRule['font-family']).toContain('Monaco');
      expect(baseRule['font-family']).toContain('Courier New');
    ...
    it('should follow Phase 6 color specifications exactly', () => {
      // Keywords
      expect(lightRules['.token.keyword']['color']).toBe(
        PHASE_6_LIGHT_COLORS.keywords
      );

      // Strings
      expect(lightRules['.token.string']['color']).toBe(
        PHASE_6_LIGHT_COLORS.strings
      );

      // Numbers
      expect(lightRules['.token.number']['color']).toBe(
        PHASE_6_LIGHT_COLORS.numbers
      );

      // Comments
      expect(lightRules['.token.comment']['color']).toBe(
        PHASE_6_LIGHT_COLORS.comments
      );
      expect(lightRules['.token.comment']['font-style']).toBe('italic');

      // Operators
      expect(lightRules['.token.operator']['color']).toBe(
        PHASE_6_LIGHT_COLORS.operators
      );
      ...
      // Attribute names
      expect(lightRules['.token.attr-name']['color']).toBe(
        PHASE_6_LIGHT_COLORS.attrNames
      );

      // Attribute values
      expect(lightRules['.token.attr-value']['color']).toBe(
        PHASE_6_LIGHT_COLORS.attrValues
      );
    });
